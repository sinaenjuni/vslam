...
RUN wget -O /temp/yaml-cpp_temp.tar.gz \
    ...
    && cmake .. -DCMAKE_INSTALL_PREFIX="/thirdparty/yaml-cpp" > /thirdparty/yaml-cpp_log.txt 2>&1 \
    && make install -j4
...
RUN wget -O /temp/eigen3_temp.tar.gz \
    ...
    && cmake .. -DCMAKE_INSTALL_PREFIX="/thirdparty/eigen3" > /thirdparty/eigen3_log.txt 2>&1 \
    && make install -j4
...
RUN wget -O /temp/lapack_temp.tar.gz \
    ...
    && cmake .. -DCMAKE_INSTALL_PREFIX="/thirdparty/lapack" > /thirdparty/lapack_log.txt 2>&1 \
    && make install -j4 
...
RUN wget -O /temp/opencv_temp.tar.gz \
    ...
    && cmake .. -DCMAKE_INSTALL_PREFIX="/thirdparty/opencv" \
     -DWITH_LAPACK=ON -DLAPACK_LIBRARIES="/thirdparty/lapack/lib/libopenblas.a" \
     -DLAPACK_INCLUDE_DIRS="/thirdparty/lapack/include/" \
     > /thirdparty/opencv_log.txt 2>&1 \
    && make install -j4 
...
RUN wget -O /temp/g2o_temp.tar.gz \
    ...
    && cmake .. -DCMAKE_INSTALL_PREFIX="/thirdparty/g2o" \
        -DEigen3_DIR="/thirdparty/eigen3/share/eigen3/cmake" > /thirdparty/g2o_log.txt 2>&1 \
    && make install -j4 
...
RUN wget -O /temp/pangolin_temp.tar.gz \
    ...
    && cmake .. -DCMAKE_INSTALL_PREFIX="/thirdparty/pangolin" \
        -DEigen3_DIR="/thirdparty/eigen3/share/eigen3/cmake" > /thirdparty/pangolin_log.txt 2>&1 \
    && make install -j4 
...
RUN wget -O /temp/dbow2_temp.tar.gz \
    ...
    && cmake .. -DCMAKE_INSTALL_PREFIX="/thirdparty/dbow2" \
        -DOpenCV_DIR="/thirdparty/opencv/lib/cmake/opencv4" > /thirdparty/dbow2_log.txt 2>&1 \
    && make install -j4 
    
FROM ubuntu:22.04
RUN apt-get update && apt-get install -y \
    # Build tools
    g++ cmake libgl1-mesa-dev libepoxy-dev \
...
# RUN apt update && apt install -y wget lsb-release software-properties-common gnupg curl \
