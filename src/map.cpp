    ...
    const cv::Mat &points4d)
{
  ...
  cv::Mat matched_kps1 = frame1->get_kps(idx_frame1);
  cv::Mat matched_kps2 = frame2->get_kps(idx_frame2);

  // check reprojection errors
  cv::Mat reprojection_error1 = proj_kps1 - frame1->get_kps(idx_frame1);
  cv::reduce(reprojection_error1, reprojection_error1, 1, cv::REDUCE_SUM2);
  reprojection_error1 = reprojection_error1.mul(frame1->get_sigma2inv(idx_frame1));

  cv::Mat reprojection_error2 = proj_kps2 - frame2->get_kps(idx_frame2);
  cv::reduce(reprojection_error2, reprojection_error2, 1, cv::REDUCE_SUM2);
  reprojection_error2 = reprojection_error2.mul(frame2->get_sigma2inv(idx_frame2));
  ...
  frame1->project(
      Matrix::to_homogeneous(Matrix::to_homogeneous(frame1->get_kpsn(idx_frame1))), ray_cur);
  ...
  frame2->project(
      Matrix::to_homogeneous(Matrix::to_homogeneous(frame2->get_kpsn(idx_frame2))), ray_ref);
  ...
  cv::Mat bad_cos_parallax = (cos_parallax < 0) | (cos_parallax > this->cos_max_parallax);

  // check scale consistency
  double scale_consistency_ratio = this->scale_consistency_factor * this->scale_factor;
  ...
  cv::Mat bad_points = bad_depth1 | bad_depth2 | bad_reprojection_cur | bad_reprojection_ref |
  ...
  for (size_t i = 0; i < points4d.rows; i++)
  {
    ...
    Map_point *map_point =
        new Map_point(PosF{point4d.val[0], point4d.val[1], point4d.val[2]}, descriptor);
    // print(descriptor);

    // double_t x = matched_kps_cur.at<double_t>(i, 0);
    // double_t y = matched_kps_cur.at<double_t>(i, 1);z

    this->add_map_point(map_point);
  }
}