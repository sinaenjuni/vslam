...
#define PRINT(...)                                                             \
  VA_GENERIC(__VA_ARGS__, PRINT5, PRINT4, PRINT3, PRINT2, PRINT1)(__VA_ARGS__)
...
namespace Misc {
void draw_kps(cv::Mat &img, std::vector<cv::KeyPoint> &kps,
              cv::Scalar color = cv::Scalar(255, 0, 0));

void draw_kps(cv::Mat &img, cv::Mat &kps,
              cv::Scalar color = cv::Scalar(255, 0, 0));

void draw_line(cv::Mat &img, const cv::Mat kps_cur, const cv::Mat kps_ref,
